...
if [[ -n "${BF_KEYSTORE:-}" ]]; then
  ...
  "$bf_tools/apksigner" sign --ks "$BF_KEYSTORE" --ks-key-alias "$BF_KEY_ALIAS" --ks-pass env:BF_KEY_PASSWORD --key-pass env:BF_KEY_PASSWORD --out out/Black-Follow-0.4.0-test.apk out/aligned.apk
  "$bf_tools/apksigner" verify --verbose out/Black-Follow-0.4.0-test.apk
else
