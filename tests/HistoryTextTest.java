...
public class HistoryTextTest {
    ...
    public static void main(String[] args)throws Exception {
        ...
        StringWriter lists=new StringWriter();HistoryText.Report lr=new HistoryText.Report(lists,"example",at("2026-03-30"),at("2026-03-30"),at("2026-01-19"),zone);
        lr.currentLists(at("2026-01-19"));lr.listHeading("followers");
        lr.person("initial","First\nName",0,0);lr.person("new_person","New",at("2026-01-21"),at("2026-01-20"));lr.listCount(2);
        lr.listHeading("following");for(int i=0;i<205;i++)lr.person("person"+i,"",0,0);lr.listCount(205);lr.finish();
        String listText=lists.toString();
        check(listText.contains("MEVCUT KİŞİ LİSTELERİ")&&listText.contains("TAKİPÇİLER")&&listText.contains("TAKİP EDİLENLER"),"both current lists exported separately");
        check(listText.contains("Son tam tarama: 19.01.2026"),"list date is not newer profile-count date");
        check(listText.contains("@initial • First Name\n  Zaman bilinmiyor"),"baseline time unknown and name cannot inject report lines");
        check(listText.contains("@new_person")&&listText.contains("Tespit aralığı: 20.01.2026"),"later addition keeps detection interval");
        check(listText.contains("https://www.instagram.com/new_person/"),"current person has profile link");
