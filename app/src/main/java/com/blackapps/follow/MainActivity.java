...
public class MainActivity extends Activity {
    ...
    private void check(long id,boolean profileOnly){
        ...
        new Thread(()->{String result=profileOnly?Monitor.profile(app,id):Monitor.run(app,id,true);runOnUiThread(()->{if(isFinishing()||isDestroyed())return;refreshDisplayedData();new AlertDialog.Builder(this).setTitle("Kontrol sonucu").setMessage(result)
            .setNeutralButton("Bilgiyi kopyala",(d,w)->{android.content.ClipboardManager cm=(android.content.ClipboardManager)getSystemService(CLIPBOARD_SERVICE);cm.setPrimaryClip(ClipData.newPlainText("Black Follow kontrol","Black Follow 0.4.3\n"+result));toast("Kontrol bilgisi kopyalandı");})
    ...
    private void drawer(){
        ...
        panel.addView(text("BLACK FOLLOW",22,GREEN));panel.addView(text("0.4.3",13,MUTED));
